...
// Type guard function to check if the response matches BannedUserError shape
function isBannedUserError(value: unknown): value is BannedUserError {
  return (
    ...
    'code' in value &&
    'message' in value &&
    (value as BannedUserError).code === 'BANNED_USER' &&
    typeof (value as BannedUserError).message === 'string'
  );
}
